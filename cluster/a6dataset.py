...
class Dataset(object):
    ...
    def __init__(self, dim, contents=None):
        ...
        #assert type(contents) == None and a6checks.is_point_list(contents)
        self._dim = dim
        if contents == None:
            self._contents = []
        else:
    ...
    def addPoint(self,point):
        ...
        #assert a6checks.addPoint_helper(self,point) == True
        mycopy = []
        for x in point:
            x=x
            mycopy.append(x)
        self._contents.append(mycopy)
